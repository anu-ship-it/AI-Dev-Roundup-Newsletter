...
// Email template include this link automatically.
    ...
    try {
        ...
        if (!token) {
            return NextResponse.json(
                { error: "Missing unsubsrcibe token" },
                { status: 400 }
            );
        }
        ...
        if (!subscriber.subscribed) {
            // Already unsubscribed - retun success anyway
        ...
        return new NextResponse(unsubscribedHTML("You've been unsubscribed sucessfully."), {
            headers: { "Content-Type": "text/html" },
        });
    } catch (error) {
...
function unsubscribedHTML(message: string) {
    ...
    <head>
        ...
        <meta charset="utf-8">
        <style>
           body { font-fmaily: -apple-system, sans-serif; display: flex; align-items: center; justify-content: center; min-height: 100vh; margin: 0; background: #f8fafc; }
                   ...
                   h1 { color: #of172a: font-size: 34px; margin: 0 0 12px; }
